...
G4bool FullModelReactionDynamics::TwoCluster(
  ...
  if (leadFlag) {
    ...
    G4ThreeVector preCurrentP = currentParticle.GetMomentum();
    G4ThreeVector preTargetP = targetParticle.GetMomentum();
    G4double preCurrentM = currentParticle.GetMass();
    G4double preTargetM = targetParticle.GetMass();
    if (dum) {
    ...
    G4ThreeVector postCurrentP = currentParticle.GetMomentum();
    G4ThreeVector postTargetP = targetParticle.GetMomentum();
    G4double postCurrentM = currentParticle.GetMass();
    G4double postTargetM = targetParticle.GetMass();

    if (preCurrentP != postCurrentP || preTargetP != postTargetP || preCurrentM != postCurrentM || preTargetM != postTargetM) {
      G4cout << "STRANGE EFFECTS IN TWOCLUSTER AFFECTED THE OUTGOING MOMENTA!" << G4endl;
      G4cout << "PreCurrentP = " << preCurrentP << " PostCurrentP = " << postCurrentP << G4endl;
      G4cout << "PreTargetP = " << preTargetP << " PostTargetP = " << postTargetP << G4endl;
      G4cout << "PreCurrentM = " << preCurrentM << " PostCurrentM = " << postCurrentM << G4endl;
      G4cout << "PreTargetM = " << preTargetM << " PostTargetM = " << postTargetM << G4endl;
      exit(EXIT_FAILURE);
    }
  }  // end of if( leadFlag )
  //
