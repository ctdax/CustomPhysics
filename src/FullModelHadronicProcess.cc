...
G4VParticleChange* FullModelHadronicProcess::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep) {
  ...
  for (ReactionProduct::iterator it = reactionProduct.begin(); it != reactionProduct.end(); ++it) {
    ...
    if (finalStateParticle->GetParticleType()=="rhadron") {
      G4cout << "Final state particle is an rhadron" << G4endl;
  ...
  //Fill the vector with the secondary particles. Here secondary particle is defined as all final state particles that are not R-Hadrons
  ...
  //Declare the Cloud 4-momentum after the interaction and propose an energy deposit of the difference between the incoming and outgoing Cloud energies
...
void FullModelHadronicProcess::CalculateMomenta(
    ...
    const G4HadProjectile* incomingCloudG4HadProjectile,  //The incoming cloud
    ...
    G4ReactionProduct& incomingCloudG4Reaction, //The outgoing particle previously defined as original incident
    G4ReactionProduct& targetParticleG4Reaction, //The outgoing particle previously defined as original target
    G4bool& incomingRhadronHasChanged, //True if the incident particle has changed
